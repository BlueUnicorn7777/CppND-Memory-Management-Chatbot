...
ChatLogic::~ChatLogic()
{
   ...
   // std::cout<< ((_chatBot!=nullptr)?"CL-T\n":"CL-F\n");
    //delete _chatBot;
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        //auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](GraphNode *node) { return node->GetID() == id; });
                          auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](const GN &node) { return node->GetID() == id; });
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            auto parentNode = std::find_if(_nodes.begin(), _nodes.end(), [&parentToken](const GN &node) { return node->GetID() == std::stoi(parentToken->second); });
                            auto childNode = std::find_if(_nodes.begin(), _nodes.end(), [&childToken](const GN &node) { return node->GetID() == std::stoi(childToken->second); });
                            // create new edge
                            GE edge = std::make_unique<GraphEdge>(id);
    ...
    //create the chatbot and move it to rootnode
